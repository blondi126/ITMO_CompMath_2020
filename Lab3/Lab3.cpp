...
double PolDel(double a, double b, double eps, double (*f) (double), int* c, double min, double max) {     // сам метод
    *c = 0;
    while (fabs(a - b) >= sqrt(2*eps*min/max)) {   // проверка на заданную точность
...
int main() {
    ...
    double x[5];
    double m[5] = { 8, -1.617, 2, 2.4, 4.5 };
    double M[5] = { 26, 44.55, 12, 11.6, 11.556 };
    for (int i = 0; i < 5; i++)
        x[i] = PolDel(a[i], b[i], epsilon, f[i], &count, m[i], M[i]);
    printf("Точность %.8f\n", epsilon);
    for (int i = 0; i < 5; i++) {
